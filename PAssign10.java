...
public class PAssign10 extends Application {
    ...
    private ObservableList<String> convert = FXCollections.observableArrayList("MPG", "KPL");
    private ComboBox<String> cmb1 = new ComboBox<>(convert);
    ...
    private void changeLabels() {
    	...
    	if (cmb1.getValue().equals("KPL") && lblCapacity.getText().equals(defaultCapacity) && tfCapacity.getText() != null && !tfCapacity.getText().isEmpty()
        		&& tfDistance.getText() != null && !tfDistance.getText().isEmpty()) {
        	// update labels & get user input
    	...
    	//convert values when user switches to KPL 
        if (cmb1.getValue().equals("KPL")) {
        	distance = (distance != 0) ? distance * 1.609 : 0;
        	capacity = (capacity != 0) ? capacity * 3.785 : 0;
        }
        //change the TextBox with the converted values
